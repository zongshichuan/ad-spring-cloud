package com.chuan.ad.sender.index;

import com.alibaba.fastjson.JSON;
import com.chuan.ad.dump.table.*;
import com.chuan.ad.handler.AdLevelDataHandler;
import com.chuan.ad.index.DataLevel;
import com.chuan.ad.constant.Constant;
...
/**
 *监听到Binlog增量数据, 数据投递(把增量数据添加到JVM内存中)
 */
@Slf4j
@Component
    ...
    private void Level4RowData(MySqlRowData rowData) {
        ...
        switch (rowData.getTableName()) {
            ...
            case Constant.AD_UNIT_IT_TABLE_INFO.TABLE_NAME:
                ...
                log.info("AdUnitIt 增量数据 start >>>");
                itTables.forEach(
                        i -> AdLevelDataHandler.handleLevel4(i, rowData.getOpType())
                );
                log.info("AdUnitIt 增量数据 end >>>");
                break;
